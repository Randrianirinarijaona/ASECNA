...
def add_item(db: Session, airport: Airport, category: NetworkCategoryEnum, title: str, status_value: str | None, description: str | None, details: list[str] | None) -> NetworkItem:
    existing = db.scalar(select(NetworkItem).where(NetworkItem.airport_key == airport.key, NetworkItem.category == category, func.lower(NetworkItem.title) == title.lower()))
    if existing:
        raise HTTPException(status.HTTP_409_CONFLICT, detail=f"Le paramètre '{title}' existe déjà pour cet aéroport")
    effective_status = None if category == NetworkCategoryEnum.sfa else (status_value or "operational")
    item = NetworkItem(airport_key=airport.key, category=category, title=title, status=effective_status, description=description, details=details)
    db.add(item)
    db.commit()
    db.refresh(item)
    return item
...
def get_item(db: Session, airport_key: str, category: NetworkCategoryEnum, title: str) -> NetworkItem | None:
    return db.scalar(select(NetworkItem).where(NetworkItem.airport_key == airport_key, NetworkItem.category == category, NetworkItem.title == title))
...
def update_status(db: Session, item: NetworkItem, status_value: str) -> NetworkItem:
    if item.category == NetworkCategoryEnum.sfa:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, detail="Les items SFA n'ont pas de statut global")
...
def get_airports_by_network(db: Session, category: NetworkCategoryEnum, sub_item: str) -> list[AirportNetworkMatchOut]:
    items = db.scalars(select(NetworkItem).where(NetworkItem.category == category).join(Airport, Airport.key == NetworkItem.airport_key)).all()
    ...
    for item in items:
        if needle in item.title.lower():
            ...
            if airport:
                results.append(AirportNetworkMatchOut(key=airport.key, matched_title=item.title, status=item.status, airport_name=airport.name, airport_iata=airport.iata))
    return results
