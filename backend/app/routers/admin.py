...
def get_stats(db: Session = Depends(get_db)):
    ...
    users_by_role = {
        role.value: (
            db.scalar(select(func.count()).select_from(User).where(User.role == role)) or 0
        )
        for role in RoleEnum
    }

    total_airports = (
        db.scalar(select(func.count()).select_from(Airport).where(Airport.is_technical_point.is_(False))) or 0
    )
    total_technical_points = (
        db.scalar(select(func.count()).select_from(Airport).where(Airport.is_technical_point.is_(True))) or 0
    )
    ...
    return AdminStats(
        ...
        total_technical_points=total_technical_points,
