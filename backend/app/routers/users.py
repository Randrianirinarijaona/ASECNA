...
def list_users(page: int = 1, page_size: int = 10, search: str = "", _: User = Depends(require_admin), db: Session = Depends(get_db)):
    items, total = user_crud.list_users(db, page, page_size, search)
    return PaginatedUsers(items=[_to_user_out(u) for u in items], total=total, page=page, page_size=page_size, total_pages=user_crud.paginate_meta(total, page, page_size))
