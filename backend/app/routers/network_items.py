...
# ─── Items d'un aéroport (NetworkModal.tsx) ──────────────────────────────
...
def add_item(
    ...
    item = network_crud.add_item(
        db, airport, category, payload.title,
        payload.status.value if payload.status else None,
        payload.description, payload.details,
    )
    user_crud.log_activity(db, current_user, f"Ajout du paramètre {payload.title} ({category.value}) - {airport_key}")
...
def delete_item(
    ...
    # Reproduit useAirportsData.deleteNetworkItem : purge aussi les liaisons associées.
    link_crud.delete_links_for_item(db, category, title, airport_key)
    network_crud.delete_item(db, item)
    user_crud.log_activity(db, current_user, f"Suppression du paramètre {title} ({category.value}) - {airport_key}")
...
def update_item(
    item_id: str,
    payload: NetworkItemUpdate,
    current_user: User = Depends(require_write_access),
    db: Session = Depends(get_db),
):
    """
    Correspond à NetworkItemModal.tsx::onUpdateItem : met à jour titre,
    statut (sauf SFA) et description en une seule opération.
    """
    ...
    db.commit()
    db.refresh(item)
    user_crud.log_activity(db, current_user, f"Modification du paramètre {item.title}")
    return _to_item_out(item)
...
def toggle_sub_parameter_status(
    sub_id: str,
    _: User = Depends(require_write_access),
    db: Session = Depends(get_db),
):
...
# ─── Usage réseau (NetworkUsageTab.tsx / getAirportsByNetwork) ──────────
